...
const Product = () => {
  ...
  const {id} = useParams()
  const endpoint = id ? `${config.baseUrl}/products/${id} ` : `${config.baseUrl}/products/` // endpoint of individual products
                  ...
                  {productData.images.map((item, index) => (
                    <img
                      ...
                      className={`w-24 cursor-pointer opacity-80 hover:opacity-100 duration-300 ${
                        imgUrl === item &&
                        "border border-gray-500 rounded-lg opacity-100"
                      }`}
                  ...
                  <div className="flex items-center gap-x-3">
                    {productData.colors.map((item) => (
                      <div
                        ...
                        className={`${
                          item === color
                            ? "border border-black p-[6px] rounded-full text-center"
                            : "border-transparent"
                        }`}
                      >  
                        <div
